...
public class VendorProductFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
    {
        ...
        View v = inflater.inflate(R.layout.fragment_vendor_product, container, false);
        ...
        {
            setAdapter();
            loadVendorProduct(arguments.getString("vendor_id"));
        }
        else
        {
            ...
            public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
                ...
                if(dy > 0)
                {
                    if (isScrolling && (currentItems + scrollOutItems == totalItems))
                    {
                        //Fetch new Data
                        isScrolling = false;
                        loadVendorProductPagination(arguments.getString("vendor_id"));
                    }
                }
            }
        });
    ...
    private void loadVendorProduct(String cat_id)
    {
        dialogue.show();
        SOService soService = ApiUtils.getSOService();
        soService.vendor_products(Utils.getSimpleTextBody("0"),
    ...
    private void loadVendorProductPagination(String cat_id)
    {
        SOService soService = ApiUtils.getSOService();
        soService.vendor_products(Utils.getSimpleTextBody(String.valueOf(totalItems)),
                Utils.getSimpleTextBody(cat_id)).enqueue(new Callback<ProductMainClass>() {
            @Override
            public void onResponse(Call<ProductMainClass> call, Response<ProductMainClass> response) {
                dialogue.cancel();
